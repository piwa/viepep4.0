 ...
 */
public class StartParNotExceedImpl extends AbstractDockerProvisioning implements ProcessInstancePlacementProblemService {
    ...
    public Result optimize(Date tau_t) {
        ...
        for (Map.Entry<Element, List<Element>> nextStepsEntry : nextSteps.entrySet()) {
            ...
            if (!runningWorkflows.contains((WorkflowElement) nextStepsEntry.getKey())) {
                ...
                if(placementHelper.getRemainingLeasingDuration(new Date(), vm) < ((ProcessStep) nextStep).getExecutionTime()) {
                    vm = startNewVm(result, dockerContainer);
                }
            }

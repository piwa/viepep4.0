 ...
 */
public class AllParNotExceedImpl extends AbstractDockerProvisioning implements ProcessInstancePlacementProblemService {
    ...
    public Result optimize(Date tau_t) {
        ...
        for (Map.Entry<Element, List<Element>> nextStepsEntry : nextSteps.entrySet()) {
            ...
            List<VirtualMachine> currentUsedVms = new ArrayList<>();
            for (Element nextStep : nextStepsEntry.getValue()) {
                ...
                if(runningVms.size() > 0) {
                    ...
                    for(VirtualMachine currentVm : runningVms) {
                        long availableExecutionTime = placementHelper.getRemainingLeasingDuration(new Date(), currentVm);
                        if(!currentUsedVms.contains(currentVm) && availableExecutionTime > ((ProcessStep) nextStep).getExecutionTime()) {
                            vm = currentVm;
                            break;
                        }
                    }

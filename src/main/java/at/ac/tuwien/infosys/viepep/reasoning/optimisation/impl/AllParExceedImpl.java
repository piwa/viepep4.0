 ...
 */
public class AllParExceedImpl extends AbstractDockerProvisioning implements ProcessInstancePlacementProblemService {
            ...
            for (Element nextStep : nextStepsEntry.getValue()) {
                ...
                if(runningVms.size() > 0) {
                    ...
                    for(VirtualMachine currentVm : runningVms) {
                        if(!currentUsedVms.contains(currentVm)) {
                            vm = currentVm;
                            break;
                        }
                    }
